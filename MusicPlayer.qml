...
Item {
    ...
    Item {
        ...
        property real progress: Mpris.players.values[playerNum]?.position ?? 0
        property real length: Mpris.players.values[playerNum]?.length ?? 0
        ...
        Rectangle {
            ...
            MouseArea {
                ...
                onReleased: {
                    ...
                    if (Mpris.players.values[playerNum])
                        Mpris.players.values[playerNum].position = (Math.min(Math.max(selectionArea.startPos, selectionArea.mouseX), selectionArea.endPos) - 8) / 312 * progressBar.length
                }
